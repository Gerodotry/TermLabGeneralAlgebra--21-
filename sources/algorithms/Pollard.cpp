...
static std::vector<long long> Pollard(long long N) {
    if (N < 1) {
    ...
    if (N % 2 == 0) {
        std::vector<long long> t = Pollard(N / 2);
        t.push_back(2);
        return t;
    }
    ...
    if (isPrime(gcd1)) {
        ...
        std::vector<long long>  t = Pollard(gcd1);
    ...
    if (isPrime(zvor)) {
        ...
        std::vector<long long>  t = Pollard(zvor);
        ans.insert(ans.end(), t.begin(), t.end());
    }
...
{
    std::vector<long long> primeNumbers = Pollard(number);
